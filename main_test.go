...
func TestRunCommand_versionAndHelp(t *testing.T) {
	cases := []struct {
		...
		{args: []string{"-help"}, wantHandled: true, wantSubstr: "Usage:"},
		{args: []string{"--help"}, wantHandled: true, wantSubstr: "Usage:"},
		{args: []string{"version"}, wantHandled: true, wantSubstr: "aws-green dev"},
		{args: []string{"-version"}, wantHandled: true, wantSubstr: "aws-green dev"},
		{args: []string{"--version"}, wantHandled: true, wantSubstr: "aws-green dev"},
	}
...
// The Homebrew formula's test block runs "aws-green --help"; it has to exit 0
...
func TestRunCommand_helpNeedsNoConfig(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "aws-green") {
